var config = {
  apiKey: "AIzaSyCKa0-NRF_xT_u-c474OqkevkAXTWq65-c",
  authDomain: "webassign6.firebaseapp.com",
  databaseURL: "https://webassign6.firebaseio.com",
  storageBucket: "webassign6.appspot.com",
};
...
function produceSingleItem(sinItemData){
  ...
  */
  ...
  product.viewBtn.click(function(){
    ...
    var messageBox = new MessageBox(currentUser, product.itemKey);  
    $("#message").append(messageBox.dom);
    $("#message").append(messageBox.inputBox);
    firebase.database().ref("messages/" + sinItemData.itemKey).on("value", function(data){
    messageBox.refresh();
      //利用for in存取
      var allItems = data.val();
      for (var itemKey in allItems ){
        allItems[itemKey].itemKey = itemKey;
        messageBox.addDialog(allItems[itemKey]);
      }
    });
 
    messageBox.inputBox.keypress(function(event){
      if(event.which == 13){
        //messageBox.addDialog({message: messageBox.inputBox.find("#dialog").val(), time:new Date().getTime(), name: currentUser.displayName, picURL: currentUser.photoURL});
        var chat = {};
        var curTime =new Date().getTime();
        chat["/messages/" + sinItemData.itemKey + "/" + curTime + "/message"]  = messageBox.inputBox.find("#dialog").val();
        chat["/messages/" + sinItemData.itemKey + "/" + curTime + "/picURL"]  = currentUser.photoURL;      
        chat["/messages/" + sinItemData.itemKey + "/" + curTime + "/name"]  = currentUser.displayName;
        firebase.database().ref().update(chat);
        messageBox.inputBox.find("#dialog").val(" ");
      }
    });
  });
